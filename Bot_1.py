...
def summa(message):
    global amount
    #Solo aceptar numeros positivos
    try:
    ...
    if amount>0:
        ...
        bot.send_message(message.chat.id, 'Выберите пару балют', reply_markup=markup)
    else: 
...
def callback1(call):
    if call.data != 'else':
        ...
        bot.send_message(call.message.chat.id, f'Получается: {round(res, 2)}. Можете засново вписать сумму')
        bot.register_next_step_handler(call.message, summa)
    else:
        bot.send_message(call.message.chat.id, 'Выберите пару значение через слэш')
...
def my_currency(message):
    try:
        ...
        res = currency.convert(amount, values[0], values[1])
        bot.send_message(message.chat.id, f'Получается: {round(res, 2)}. Можете засново вписать сумму')
        bot.register_next_step_handler(message, summa)
    #Cualquier excepcion
    except Exception:
